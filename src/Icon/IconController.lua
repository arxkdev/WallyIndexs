...
alignmentDetails["left"] = {
	startScale = 0,
	getOffset = function()
		local offset = 0 + IconController.leftOffset
...
IconController.leftOffset = IconController.leftGap
IconController.rightOffset = -47
...
function IconController.updateTopbar()
	local function getIncrement(otherIcon, alignment)
		...
		local alignmentGap = IconController[alignment.."Gap"]*2
	...
	task.defer(function()
		...
		for alignment, alignmentInfo in pairs(alignmentDetails) do
			...
			for i, otherIcon in pairs(records) do
                local container = otherIcon.instances.iconContainer
                container.AnchorPoint = Vector2.new(0,.5)
				local increment, preOffset = getIncrement(otherIcon, alignment)
	...
	if IconController.controllerModeEnabled then
		if bool then
			...
			if forceBool then
				...
				indicator.Image = userInputService:GetImageForKeyCode(Enum.KeyCode.ButtonB)
			...
			indicator.Image = userInputService:GetImageForKeyCode(Enum.KeyCode.DPadDown)
			indicator:TweenPosition(
				UDim2.new(0.5,0,0,5),
				Enum.EasingDirection.Out,
				Enum.EasingStyle.Quad,
...
coroutine.wrap(function()
	...
	userInputService.InputBegan:Connect(function(input,gpe)
		...
		elseif input.KeyCode == Enum.KeyCode.ButtonB and not IconController.disableButtonB then
			if IconController.activeButtonBCallbacks == 1 and TopbarPlusGui.Indicator.Image == userInputService:GetImageForKeyCode(Enum.KeyCode.DPadDown) then
				IconController.activeButtonBCallbacks = 0
